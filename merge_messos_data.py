        ...
        if tables_data["bonds"]:
            for bond in tables_data["bonds"]:
                security = {
                    ...
                    "price": bond["Price"] if bond["Price"] != "N/A" else "0",
                    "value": bond["Value (USD)"] if bond["Value (USD)"] != "N/A" else "0",
                    "weight": bond["Weight (%)"] if bond["Weight (%)"] != "N/A" else "0"
                }
                document["financial_data"]["portfolio"]["securities"].append(security)
                document["financial_data"]["portfolio"]["summary"]["total_securities"] += 1
        ...
        if tables_data["structured_products"]:
            for product in tables_data["structured_products"]:
                security = {
                    ...
                    "price": product["Price"] if product["Price"] != "N/A" else 0,
                    "value": product["Value (USD)"] if product["Value (USD)"] != "N/A" else 0,
                    "weight": product["Weight (%)"] if product["Weight (%)"] != "N/A" else 0
                }
                document["financial_data"]["portfolio"]["securities"].append(security)
                document["financial_data"]["portfolio"]["summary"]["total_securities"] += 1
