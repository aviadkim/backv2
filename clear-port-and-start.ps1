...
function Stop-ProcessOnPort {
    ...
    $connections = Get-ProcessOnPort -Port $Port
    ...
    if ($connections) {
        ...
        foreach ($processId in $processIds) {
            try {
                ...
                Write-Host "Stopping process $processName (ID: $processId) on port $Port..." -ForegroundColor Yellow
        ...
        $stillInUse = Get-ProcessOnPort -Port $Port
        if ($stillInUse) {
            Write-Host "Port $Port is still in use after termination attempt:" -ForegroundColor Red
...
$port3002Cleared = Stop-ProcessOnPort -Port 3002
...
if (-not $portAvailable) {
    ...
    foreach ($port in $alternativePorts) {
        ...
        $inUse = Get-ProcessOnPort -Port $port
        ...
        if (-not $inUse) {
            ...
            $cleared = Stop-ProcessOnPort -Port $port
            if ($cleared) {
                $selectedPort = $port
                $portAvailable = $true
