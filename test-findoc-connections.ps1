# Comprehensive FinDoc Connection Test Script
Write-Host "=== FinDoc Connection Test Suite ===" -ForegroundColor Cyan

# Configuration
$backendUrl = "http://localhost:24125"
$frontendUrl = "http://localhost:3002"
$testOutputFile = "findoc-connection-test-results.txt"
...
if ($documentsResponse -ne $null) {
    ...
    if ($documents.Count -gt 0) {
        ...
        Test-ApiEndpoint -endpoint "/api/documents/$firstDocId" -testName "Get Document by ID" -validation {
            param($response)
            return $response.document -ne $null
        }
    }
}
...
# 5. Frontend URL Tests
Write-Host "`n5. Frontend URL Tests" -ForegroundColor Yellow
$frontendPages = @(
    "/",
    "/upload",
    "/analysis",
